...
import {
  ...
  TimePicker,
  Select,
} from 'modules/DynamicForm/components'
...
function MyApp() {
  ...
  console.log('watch', aboutForm.watch())
  ...
  return (
    <FormProvider {...aboutForm}>
      <Box className="p-3">
        ...
        <DateTimePicker name="datetime" label="Datetime" />
        <TimePicker name="time" label="Time" />
      </Box>

      <Box className="p-3">
        <Select name="select" label="Select" options={top100Films} multiple />
      </Box>
    </FormProvider>
  )
}
